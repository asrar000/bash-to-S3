...
deploy() {
    ...
    log_info "Initialized deployer for bucket: ${S3_BUCKET}, prefix: ${S3_PREFIX}"

    # ── Type 1: Upload individual files directly ──────────────────────────────
    log_info "Starting direct file uploads..."
    for file in "${FILES_TO_UPLOAD[@]}"; do
        local local_path="${PROCESSOR_DIR}/${file}"
        local s3_key="${S3_PREFIX}/${file}"

        if ! upload_file "${local_path}" "${s3_key}"; then
            all_successful=false
        fi
    done

    # ── Type 2: Create zip and upload ─────────────────────────────────────────
    log_info "Starting zip creation and upload..."
    local temp_dir
    temp_dir="$(mktemp -d)"
    ...
    local zip_path
    zip_path="$(create_zip "${ZIP_NAME}" "${temp_dir}" "${ITEMS_TO_ZIP[@]}")"
    local s3_key="${S3_PREFIX}/${ZIP_NAME}"

    if ! upload_file "${zip_path}" "${s3_key}"; then
        all_successful=false
    fi
    ...
    if [[ "${all_successful}" == true ]]; then
        ...
        log_error "Deployment failed due to one or more errors"
        return 1
    fi
}
...
# -----------------------------------------------------------------------------
if ! deploy; then
    log_critical "Deployment encountered errors. Check log: ${LOG_FILE}"
    exit 1
fi
